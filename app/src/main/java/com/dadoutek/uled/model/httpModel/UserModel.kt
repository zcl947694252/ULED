...
object UserModel {
    fun updateModeStatus(): Observable<Response<Any>> {
        var mode = if (Constants.IS_ROUTE_MODE) 1 else 0
        LogUtils.v("zcl-----------获取状态更新模式-------${Constants.IS_ROUTE_MODE}---$mode----${Constants.IS_OPEN_AUXFUN}")
        return NetworkFactory.getApi()
                .updateAllModeStatus(Constants.IS_OPEN_AUXFUN, mode)
