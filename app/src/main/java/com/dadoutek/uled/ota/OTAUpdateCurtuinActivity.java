...
//            startScan();
    ...
    private void login() {
        ...
        if (lastUser==null)
            return;
        String meshName = lastUser.getControlMeshName();
...
/*    private float getVersionValue(String version) {
    ...
    private DbCurtain getLightByMeshAddress(int meshAddress) {
...
//                onScanComplet();
    ...
    private void onLeScan(LeScanEvent event) {
        ...
        if (this.mode == MODE_OTA) {
            DbCurtain light = getLightByMeshAddress(deviceInfo.meshAddress);
