 ...
 */
public class OTASwitchActivity extends TelinkMeshErrorDealActivity implements EventListener<String> {
    ...
    private List<DbSwitch> onlineLights;
    private DbSwitch dbLight;
    private Mesh mesh;
...
////                    ((ScrollView) tv_log.getParent()).fullScroll(ScrollView.FOCUS_DOWN);
    ...
    private void initData() {
        dbLight = (DbSwitch) getIntent().getSerializableExtra(Constants.UPDATE_LIGHT);
...
//        TelinkLightApplication.Companion.getApp().addEventListener(LeScanEvent.LE_SCAN_COMPLETED, this);
    ...
    private void start() {
        ...
        for (DbSwitch light : onlineLights) {
...
//            } else {
            ...
            DbSwitch light = getLightByMeshAddress(curMeshAddress);
        ...
        if (this.mode == MODE_COMPLETE || this.mode == MODE_IDLE) {
            ...
            if (mCancelBuilder == null) {
                ...
                mCancelBuilder.setPositiveButton(getString(android.R.string.ok), (dialog, which) -> {
                    ...
                    Mesh mesh = TelinkLightApplication.Companion.getApp().getMesh();
                    mesh.setOtaDevice(null);
                    mesh.saveOrUpdate(OTASwitchActivity.this);
                    dialog.dismiss();
                    finish();
                });
...
/*    private float getVersionValue(String version) {
    ...
    private DbSwitch getLightByMeshAddress(int meshAddress) {
...
//                onScanComplet();
    ...
    private void onLeScan(LeScanEvent event) {
        ...
        if (this.mode == MODE_OTA) {
            DbSwitch light = getLightByMeshAddress(deviceInfo.meshAddress);
