...
class MultiItemAdapter(data: ArrayList<MultiRegionBean>) : BaseMultiItemQuickAdapter<MultiRegionBean, BaseViewHolder>(data) {
    ...
    init {
        ...
        addItemType(Constants.REGION_TYPE, R.layout.template_recycleview)
        addItemType(Constants.REGION_AUTHORIZE_TYPE,R.layout.template_recycleview)
    }
    ...
    override fun convert(helper: BaseViewHolder?, item: MultiRegionBean?) {
        ...
        when(item!!.itemType){
            ...
            Constants.REGION_AUTHORIZE_TYPE->{
