 ...
 * 更新描述
 */
class SceneRecycleGridAdapter(layoutResId: Int, data: List<DbScene>?) : BaseItemDraggableAdapter<DbScene, BaseViewHolder>(layoutResId, data) {
    ...
    override fun convert(helper: BaseViewHolder, scene: DbScene?) {
        if (scene != null) {
            ...
            groupNum.text =  if (showGroupList!!.size > 0)
                TelinkLightApplication.getApp().getString(R.string.total) + showGroupList!!.size + TelinkLightApplication.getApp().getString(
                    R.string.piece) +
                        TelinkLightApplication.getApp().getString(R.string.group)
            else
                TelinkLightApplication.getApp().getString(R.string.total) + 0 + TelinkLightApplication.getApp().getString(R.string.piece) + TelinkLightApplication.getApp().getString(
                    R.string.group)

            helper.setVisible(R.id.template_device_card_delete,false)
            ...
            helper.setText(R.id.template_device_group_name, scene.name)
                ...
                .setVisible(R.id.template_device_setting,false)
