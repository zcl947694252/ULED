        ...
        if (group != null) {
         ...
         //   else
            ...
            when {
                group.deviceCount > 0 -> {
                    if (group.meshAddr == 0xffff) { //所有组允许点击
                        ...
                        if (group.deviceType != Constants.DEVICE_TYPE_CURTAIN)
                            helper.addOnClickListener(R.id.template_device_icon)
                    }
            ...
            if (helper.adapterPosition != 0)
                ...
                if (group.deviceType == Constants.DEVICE_TYPE_LIGHT_NORMAL || group.deviceType == Constants.DEVICE_TYPE_LIGHT_RGB || group.deviceType == Constants.DEVICE_TYPE_NO)
