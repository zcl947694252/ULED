...
//                    LogUtils.v("chown  READ --- command.data" + Arrays.bytesToHexString(command.data,","));
                    break;
                case WRITE:
                    this.postCommandTimeoutTask();
                    this.writeCharacteristic(commandContext, command.serviceUUID,
                            ...
                            command.data);
//                    LogUtils.v("chown  WRITE --- command.data" + Arrays.bytesToHexString(command.data,","));
                ...
                case WRITE_NO_RESPONSE:
                    ...
                    this.writeCharacteristic(commandContext, command.serviceUUID,
                            ...
                            command.data);//获取原始数据
//                    LogUtils.v("chown WRITE_NO_RESPONSE--- command.data" + Arrays.bytesToHexString(command.data,","));
                    break;
                case ENABLE_NOTIFY:
                    this.enableNotification(commandContext, command.serviceUUID,
                            command.characteristicUUID);
                    LogUtils.v("chown ENABLE_NOTIFY--- command.data" + Arrays.bytesToHexString(command.data,","));
                    break;
                case DISABLE_NOTIFY:
                    this.disableNotification(commandContext, command.serviceUUID,
                            command.characteristicUUID);
                    LogUtils.v("chown DISABLE_NOTIFY--- command.data" + Arrays.bytesToHexString(command.data,","));
        ...
        if (commandContext != null) {
            ...
            if (callback != null) {
                callback.success(this, command,
                        data);
//                LogUtils.v("chown -- data: "+ data.toString());
            }
        }
    }
...
//        LogUtils.v("chown ================================ 写入蓝牙的数据" + Arrays.bytesToHexString(data,","));
        if (gatt == null)
            return;
        BluetoothGattService service = this.gatt.getService(serviceUUID);
...
//            LogUtils.v("chown ==================================data" + s);
    ...
    public void sendData(byte[] data, BluetoothGattCharacteristic characteristicData,
        ...
        LogUtils.v("chown ================== sendData "+ data.toString());
...
//        LogUtils.v("chown=========================descriptor"+descriptor.toString());
